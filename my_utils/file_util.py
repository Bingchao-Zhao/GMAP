...
def pickle_writer(filename, context, mod='wb',create_dir=False):
    if create_dir:
        just_dir_of_file(filename)
    a_file = open(filename, mod)
    pickle.dump(context, a_file)
    a_file.close()

def pickle_loader(filename, mod='rb'):
